...
2. Identify similarities and differences among the units, grouping similar ones into subcategories based on common themes or patterns. Also, look for underlying themes or patterns that might not be immediately obvious.
...
2. Identify similarities and differences among the subcategories, grouping similar ones into broader, overarching categories based on common themes or patterns. Look for underlying themes or patterns that might not be immediately obvious. Ensure that each category and subcategory maintains a clear link back to the original data, preserving the context and meaning.
...
5. Ensure that each category is conceptually coherent and that the subcategories within each category are closely related. Avoid redundancy and ensure each category is distinct and meaningful.
6. Document the rationale behind each category for transparency and future reference.
...
5. Ensure the subcategories retains their meaning and relevance to the research questions or objective.
4. Assess the consistency and coherence of the subcategories.
5. Document the rationale for each subcategory assignment to ensure transparency and future reference.
6. Ensure each subcategory has a clear and concise definition, capturing its essence and criteria for inclusion.
...
2. Identify similarities and differences among the units, grouping similar ones into refined subcategories based on common and underlying themes or patterns.
...
2. Identify similarities and differences among the subcategories, grouping similar ones into broader, overarching categories based on common and underlying themes or patterns. Ensure that each category and subcategory maintains a clear link back to the original data, preserving the context and meaning.
3. Assign descriptive labels to each category that capture the essence of the grouped subcategories.
4. Ensure each subcategory is assigned to only one category. One category can contain multiple subcategories.
...
2. Identify themes and patterns by grouping similar categories into broader, overarching theoretical concepts based on common themes or patterns. Ensure each concept maintains a clear link back to the original data, preserving context and meaning.
...
Example JSON output: <---- NOT FINALIZED, does not match
Example JSON output:
{
